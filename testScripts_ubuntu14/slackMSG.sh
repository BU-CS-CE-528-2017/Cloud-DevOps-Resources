...
curl -X POST -H 'Content-type: application/json' --data "{\"text\":\"The Hadoop + Slack Cluster has been deployed - 1 master/$numSlaves | $linkToVM\", \"attachments\":[{\"text\":\"$(./haddopReport.sh;./sparkReport.sh)\"}]}" \
